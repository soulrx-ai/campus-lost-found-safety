...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AuthenticatedShell key={pathname} pathname={pathname}>
      {children}
    </AuthenticatedShell>
  );
}
...
function AuthenticatedShell({
  ...
  useEffect(() => {
    ...
    async function loadShell() {
      ...
      const [
        { data: profileData },
        { data: notificationData, error: listError },
  ...
  async function markNotificationRead(id: string) {
    ...
    if (!target || target.is_read || !profile || pendingReads.current.has(id))
      return;
            ...
            >
              <div>
                ...
                {notificationsOpen && (
                  <div
                    ...
                    className="absolute right-0 top-12 z-50 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] shadow-xl"
                  >
                    ...
                    {notificationError && (
                      <p
                        role="alert"
                        className="px-4 py-3 text-sm text-[var(--danger)]"
                      >
                        <AppMessage text={notificationError} />
                      </p>
                    )}
                      ...
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notification) => (
                          <button
                            ...
                            className={`block w-full border-b border-[var(--border)] px-4 py-3 text-left transition last:border-b-0 hover:bg-[var(--surface-soft)] ${
              ...
              <div>
                ...
                {profileOpen && (
                  <div
                    id="account-panel"
                    className="absolute right-0 top-12 z-50 w-64 max-w-[calc(100vw-2rem)] overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] shadow-xl"
                  >
                    ...
                    {logoutError && (
                      <p
                        role="alert"
                        className="px-4 pb-3 text-xs text-[var(--danger)]"
                      >
                        <AppMessage text={logoutError} />
                      </p>
                    )}
                  </div>
                )}
      ...
      <div className="flex-1">
        {pathname !== "/" && (
          <div className="app-container pt-3">
            <Link
              href="/"
              className="inline-flex min-h-9 items-center gap-2 rounded-md text-sm text-[var(--foreground-muted)] transition hover:text-[var(--success)]"
            >
